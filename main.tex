...
\restrictedartifactpanel{assets/artifact-previews/cover-showcase-evidence-strip.png}{0.35\linewidth}{0.50in}{Restricted showcase evidence strip omitted in the public build.}%
\hfill
\begin{minipage}[c]{0.61\linewidth}
\centering\small
\textbf{Ten practical resources for teachers, families, and support teams}\\[0.035in]
...
\node[stage, fill=SoftBlue] (notice) {\textbf{NOTICE ACCESS}\\sensory setting, peers,\\behavior context\\\textit{Resources 1--3}};
\node[stage, fill=SoftGold, right=0.16in of notice] (record) {\textbf{RECORD MEANING}\\student voice and\\personal evidence\\\textit{Resources 4--5}};
\node[stage, fill=SoftGreen, right=0.16in of record] (keep) {\textbf{KEEP ROUTES OPEN}\\AAC, backup modes,\\flexible roles\\\textit{Resources 6, 8, 10}};
\node[stage, fill=SoftRose, right=0.16in of keep] (return) {\textbf{RETURN FOR ACTION}\\day stories and\\family communication\\\textit{Resources 7, 9}};
\draw[route] (notice) -- (record);
\draw[route] (record) -- (keep);
\draw[route] (keep) -- (return);
\node[draw=MidTeal, fill=white, rounded corners=2pt, below=0.17in of $(record.south)!0.5!(keep.south)$,
  minimum width=3.20in, minimum height=0.38in, align=center, font=\scriptsize] {\textbf{Puzzle Plan + EQUITAS:} whose evidence survives, and what does it change?};
\end{tikzpicture}

\vspace{0.10in}
\parbox{0.86\textwidth}{\centering\small
Read the collection as a feedback path: examine conditions, preserve learner meaning, keep communication available, and return evidence to the people who can change the next opportunity.}
\end{center}
\clearpage
